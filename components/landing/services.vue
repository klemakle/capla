<template>
    <div>
        <div class="flex flex-col xl:flex-row bg-gray-50 mx-auto mb-12 mt-8 lg:my-12 md:my-20 py-10 px-10 w-full ">
            <div class="w-full md:w-2/3 lg:w-1/2  h-20 xl:h-28 px-4 py-2 md:px-2 text-gray-600">
              <p class="text-lg md:text-xl lg:text-3xl">Ce que nous faisons au <span class="text-indigo-500 ">Capla</span></p>
            </div>
            <div class="flex flex-col md:flex-row flex-nowrap">
                <div v-for="(n,index) in type_anglais" :key="index" class="bg-white card border-0 border-gray-200 flex flex-col rounded-xl py-2 px-4 md:px-2 lg:px-4 justify-start mb-2 w-full md:w-full lg:w-1/2 mx-2 ease-in duration-200 hover:shadow-md">
                    ...
                    <div class="title text-xs md:text-sm lg:text-base items-start  text-gray-600 my-1">{{n.text}}</div>
                    <div class="text-gray-400 my-1 w-full items-start text-xs lg:text-sm">{{n.description}}</div>
                </div>
            </div>
        </div>
    </div>
...
<script>
import {services} from '../../assets/js/text';
